...
function equiv$a(xs, ys) {
  return !!satisfies(ISequential, xs) === !!satisfies(ISequential, ys) && count$e(xs) === count$e(ys) && equiv$b(first$f(xs), first$f(ys)) && equiv$b(next(xs), next(ys));
}
...
function merge$6(self, ...xs) {
  return count$e(xs) ? merge$7.apply(null, Array.from(xs)) : null;
}
...
function takeLast(n, coll) {
  return n ? drop(count$e(coll) - n, coll) : emptyList();
}
...
function partition3(n, step, xs) {
  ...
  return n === count$e(part) ? cons(part, partition3(n, step, drop(step, coll))) : emptyList();
}
...
function partition4(n, step, pad, xs) {
  ...
  if (!coll) return xs;
  const part = take2(n, coll);
  return n === count$e(part) ? cons(part, partition4(n, step, pad, drop(step, coll))) : cons(take2(n, concat(part, pad)));
}
...
function partitionBy(f, xs) {
  ...
  return cons(run, partitionBy(f, seq$g(drop(count$e(run), coll))));
}
...
function scan(n, xs) {
  return lazySeq((function() {
    const ys = take2(n, xs);
    return count$e(ys) === n ? cons(ys, scan(n, rest$f(xs))) : emptyList();
  }));
}
...
function randNth2(random = Math.random, coll) {
  return nth$6(coll, randInt(random, count$e(coll)));
}
...
function cond(...xs) {
  const conditions = isEven(count$e(xs)) ? xs : Array.from(concat(butlast(xs), [ constantly(true), last(xs) ]));
  return function(...args) {
...
function indexedSeq2(seq, start) {
  return start < count$e(seq) ? new IndexedSeq(seq, start) : emptyList();
}
...
function contains$8(self, key) {
  return key < count$e(self.seq) - self.start;
}
...
function count$c(self) {
  return count$e(self.seq) - self.start;
}
...
function nth$3(self, idx) {
  return nth$6(self.coll, count$b(self) - 1 - idx);
}
...
function rest$9(self) {
  return count$8(self) > 0 ? indexedSeq(self, 1) : emptyList();
}

function seq$d(self) {
  return count$8(self) ? self : null;
}
...
function leaves(self) {
  return remove(comp(count$e, children), descendants(self));
}
...
function flushable(self) {
  return count$e(self.history) > 1;
}

const crunchable = flushable;

function crunch(self) {
  return crunchable(self) ? new Journal(self.pos, self.max, toArray(splice(self.history, count$e(self.history) - 1, 1, [])), self.state) : self;
}

function undoable(self) {
  return self.pos + 1 < count$e(self.history);
}
...
function revert(self) {
  const at = count$e(self.history) - 1, state = nth$6(self.history, at);
  return new Journal(at, self.max, self.history, state);
}

function revertible(self) {
  return self.pos !== count$e(self.history) - 1;
}
...
function equiv$6(self, other) {
  return descriptive(other) && count$e(keys$c(self)) === count$e(keys$c(other)) && reduce$7((function(memo, key) {
...
function seq$c(self) {
  return count$e(self) ? map((function(key) {
...
const count$7 = comp(count$e, keys$c);
...
function equiv$5(self, other) {
  ...
  return self.constructor === other.constructor && count$e(self) === count$e(other) && reducekv$5((function(memo, key, value) {
...
function reFind(re, s) {
  ...
  if (matches) {
    return count$e(matches) === 1 ? first$f(matches) : matches;
  }
}
...
function reSeq(re, s) {
  return lazySeq((function() {
    const matchData = reFind(re, s), matchIdx = s.search(re), matchStr = isArray(matchData) ? first$f(matchData) : matchData, postIdx = matchIdx + max(1, count$e(matchStr)), postMatch = s.substring(postIdx);
...
function rePattern(s) {
  ...
  const found = reFind(/^\(\?([idmsux]*)\)/, s), prefix = get(found, 0), flags = get(found, 1), pattern = s.substring(count$e(prefix));
...
function equiv$1(self, other) {
  return count$e(self) === count$e(other) && reduce$7((function(memo, value) {
...
function seq$5(self) {
  return count$e(self.coll) ? map(first$f, seq$g(self.coll)) : null;
}
...
addMethod(coerce, [ Array, HashSet ], (_into4 = into, _set = set([]), function into(_argPlaceholder10) {
  return _into4(_set, _argPlaceholder10);
}));
...
addMethod(coerce, [ Array, Object ], (_into5 = into, _param5 = {}, function into(_argPlaceholder11) {
  return _into5(_param5, _argPlaceholder11);
}));
